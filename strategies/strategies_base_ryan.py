...
class MovingAverageStrategy(Strategy):
    ...
    def __init__(self, short_window: int = 20, long_window: int = 60, position_size: float = 10.0):
        ...
        # Shorter rolling mean period
        self.short_window = short_window
        #Longer rolligng mean period
        self.long_window = long_window
        #How many options/stocks to trade
        self.position_size = position_size

    def add_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        #Makes a new column of the rolling mean for the shorter time frame
        df["MA_short"] = df["Close"].rolling(self.short_window, min_periods=1).mean()
        # Makes a new column of the rolling mean for the longer time frame
        df["MA_long"] = df["Close"].rolling(self.long_window, min_periods=1).mean()
        # Makes a new column for the returns from the row before to now
        df["returns"] = df["Close"].pct_change().fillna(0.0)
        # Calculates the volatility 
        df["volatility"] = df["returns"].rolling(self.long_window).std().fillna(0.0)
        
        #returns the dataframe with the curreent edits
        return df

    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        #Creates new columns for whether to buy or sell
        df["signal"] = 0

        #Says conditions necessary to buy or sell
        ...
        #Based on data above, changes data to tell if one should buy or sell
        df.loc[buy, "signal"] = 1
        df.loc[sell, "signal"] = -1

        #Create 'position' column that says to buy or sell 
        df["position"] = 0
        df.loc[df["MA_short"] > df["MA_long"], "position"] = 1
        df.loc[df["MA_short"] < df["MA_long"], "position"] = -1
        
        #Creates 'target_qty' column which says how much we want to buy/sell
